...
-- ============================================================
-- Moneda de la boleta (cómo se MUESTRA el monto). La orden sigue
-- siempre en la moneda principal (Estadísticas siempre en esa moneda);
-- esto solo cambia la boleta: 'principal' (US$), 'secundaria' (pesos,
-- convertido) o 'ambas'. Ver boleta_moneda_supabase.sql.
-- ============================================================
...
-- Aclaraciones del técnico en la boleta (flag incluir sí/no). Ver
-- servicio_aclaraciones_supabase.sql (incluye el create or replace de
-- boleta_publica con aclaraciones_tecnico + incluir_aclaraciones_tecnico).
...
-- ============================================================
-- Plan de ahorro: sección propia (separada de Proveedores). Mismo patrón
-- de cuenta corriente (saldo = suma de movimientos, nunca se guarda), en
-- tabla propia para no mezclar los dos negocios.
-- ============================================================
...
-- que boleta_publica (token público + RPC security definer), aplicado a los
-- pagos de Plan de ahorro.
...
-- negocio a admin_listar_negocios(), para ver "hace cuánto pagó" sin
-- tener que ir a buscarlo aparte.
drop function if exists admin_listar_negocios();
...
-- y le faltaban 3 tornillos", con foto).
...
-- Enlace público de stock en tiempo real, uno por negocio. Apagado por
-- defecto — el dueño lo prende desde Configuración cuando quiera. Cada
-- dispositivo se puede ocultar del público individualmente.
...
-- repuesto (mismo patrón que productos.cantidad/costo para accesorios).
-- reparaciones_repuestos registra qué repuestos se consumieron en cada
-- reparación, con el costo "congelado" al momento de usarlo.
...
-- ============================================================
-- Panel Admin: historial de comprobantes por negocio. Ver
-- admin_historial_comprobantes_supabase.sql, que es el patch real a
-- correr — admin_listar_comprobantes_pendientes() solo mostraba los
-- pendientes; una vez aprobado/rechazado un comprobante no había forma
-- de volver a verlo desde /admin (aunque el dato seguía intacto).
-- ============================================================
...
-- ============================================================
-- Panel Admin PRO: rediseno completo. Ver admin_panel_pro_supabase.sql,
-- que es el patch real a correr -- agrega auditoria global de admin
-- (admin_auditoria), notas/etiquetas administrativas por negocio,
-- registro de auditoria en las funciones admin_* que ya existian,
-- directorio paginado/filtrable/ordenable, ficha de negocio completa,
-- pagos globales, metricas de dashboard y centro de alertas.
-- ============================================================
