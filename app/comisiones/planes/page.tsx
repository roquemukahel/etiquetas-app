...
function describirRegla(r: Regla, moneda: string, productos: Producto[]): string {
  const donde =
    r.alcance === 'todas' ? 'todas las ventas'
    : r.alcance === 'minorista' ? 'las ventas minoristas'
    : r.alcance === 'mayorista' ? 'las ventas mayoristas'
    : r.alcance === 'tipo_item' ? (TIPO_ITEM_LABEL[r.tipo_item || ''] ?? 'un tipo de ítem')
    : `el producto ${productos.find((p) => p.id === r.producto_id)?.nombre ?? ''}`.trim();

  if (r.tipo_calculo === 'porcentaje_venta') return `${r.valor}% sobre la venta de ${donde}`;
  if (r.tipo_calculo === 'porcentaje_ganancia') return `${r.valor}% sobre la ganancia de ${donde}`;
  if (r.tipo_calculo === 'fijo_por_unidad') return `${moneda}${r.valor} por unidad (${donde})`;
  return `${moneda}${r.valor} por venta (${donde})`;
}
...
export default function PlanesComisiones() {
  ...
  const agregar = async () => {
    ...
    if (!v || v <= 0) return setError('Ingresá un valor válido.');
    if (alcance === 'producto' && !productoId) return setError('Elegí un producto.');
    setGuardando(true);
    setError(null);
  ...
  const eliminar = async (id: string) => {
    if (!planId || !puede) return;
    if (!confirm('¿Eliminar esta regla? Las comisiones ya generadas no cambian.')) return;
  ...
  if (!loading && !puede) {
    return (
      <main className="flex min-h-screen flex-col items-center justify-center gap-3 px-6 text-center">
        <p className="text-sm text-muted dark:text-dark-text-secondary">No tenés permiso para configurar reglas.</p>
  ...
  return (
    <main className="flex min-h-screen flex-col px-6 py-6 gap-4 max-w-2xl mx-auto w-full">
      <header className="flex items-center gap-3">
        ...
        <span className="text-lg font-medium">Reglas de comisión</span>
      </header>
      ...
      {loading ? (
        ...
        <div className="rounded-2xl border border-border dark:border-dark-border bg-white dark:bg-dark-surface p-5 text-center flex flex-col items-center gap-2">
          <p className="text-sm">Primero activá las comisiones y poné los porcentajes básicos.</p>
          <Link href="/configuracion/comisiones" className="rounded-xl bg-accent dark:bg-dark-accent text-white px-4 py-2 text-xs font-medium">Ir a configuración</Link>
        </div>
      ) : (
        <>
          <p className="text-sm text-muted dark:text-dark-text-secondary">
            Podés tener una comisión distinta según lo que se venda. Cuando una venta encaja en varias reglas, se aplica
            la <strong>más específica</strong>: un producto puntual gana sobre un tipo de ítem, y un tipo de ítem gana
            sobre la regla general de minorista/mayorista.
          </p>
          ...
          <div className="rounded-2xl bg-white dark:bg-dark-surface border border-border dark:border-dark-border shadow-card divide-y divide-border dark:divide-dark-border">
            {reglas.length === 0 ? (
              <p className="text-sm text-muted dark:text-dark-text-secondary text-center py-6">Sin reglas todavía.</p>
            ) : (
              reglas.map((r) => (
                <div key={r.id} className="flex items-center gap-3 px-4 py-3">
                  <p className="text-sm flex-1">{describirRegla(r, moneda, productos)}</p>
                  <button onClick={() => eliminar(r.id)} className="text-xs text-bad hover:underline shrink-0">Eliminar</button>
                </div>
              ))
            )}
          </div>
          ...
          <div className="rounded-2xl bg-white dark:bg-dark-surface border border-border dark:border-dark-border shadow-card p-4 flex flex-col gap-3">
            <p className="text-sm font-semibold">Agregar una regla</p>

            <label className="text-xs text-muted dark:text-dark-text-secondary">Se aplica a</label>
            <select value={alcance} onChange={(e) => setAlcance(e.target.value as AlcanceRegla)} className="bg-canvas dark:bg-dark-bg border border-border dark:border-dark-border rounded-lg px-3 py-2 text-sm">
              <option value="tipo_item">Un tipo de ítem (dispositivos / accesorios / trabajos)</option>
              <option value="producto">Un producto puntual del catálogo</option>
              <option value="minorista">Todas las ventas minoristas</option>
              <option value="mayorista">Todas las ventas mayoristas</option>
            ...
            {alcance === 'tipo_item' && (
              <select value={tipoItem} onChange={(e) => setTipoItem(e.target.value)} className="bg-canvas dark:bg-dark-bg border border-border dark:border-dark-border rounded-lg px-3 py-2 text-sm">
                <option value="dispositivo">Dispositivos (celulares)</option>
                <option value="producto">Accesorios</option>
            ...
            <label className="text-xs text-muted dark:text-dark-text-secondary">Cómo se calcula</label>
            <select value={tipoCalculo} onChange={(e) => setTipoCalculo(e.target.value as TipoCalculo)} className="bg-canvas dark:bg-dark-bg border border-border dark:border-dark-border rounded-lg px-3 py-2 text-sm">
              {(Object.keys(LABEL_TIPO_CALCULO) as TipoCalculo[]).map((tc) => (
                <option key={tc} value={tc}>{LABEL_TIPO_CALCULO[tc]}</option>
              ))}
            </select>

            <div className="flex items-center gap-2">
              <input value={valor} onChange={(e) => setValor(e.target.value.replace(',', '.').replace(/[^\d.]/g, ''))} inputMode="decimal" placeholder={tipoCalculo.startsWith('porcentaje') ? 'Porcentaje' : 'Monto'} className="flex-1 bg-canvas dark:bg-dark-bg border border-border dark:border-dark-border rounded-lg px-3 py-2 text-sm" />
