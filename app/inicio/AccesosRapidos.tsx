...
function datosDe(href: string, m: MetricasAccesos): { badges: { texto: string; tono: Tono }[]; nota?: string } {
  switch (href) {
    case '/ordenes':
      return {
        badges: [{ texto: `${m.ordenesPendientes} ${plural(m.ordenesPendientes, 'pendiente', 'pendientes')}`, tono: m.ordenesPendientes > 0 ? 'warn' : 'info' }],
        nota: m.ultimaVentaTexto ?? undefined,
      };
    case '/stock':
      return {
        badges: [
          { texto: `${m.enStock} en stock`, tono: 'info' },
          ...(m.porAgotarse > 0 ? [{ texto: `${m.porAgotarse} por agotarse`, tono: 'warn' as Tono }] : []),
          ...(m.sinPrecio > 0 ? [{ texto: `${m.sinPrecio} sin precio`, tono: 'warn' as Tono }] : []),
        ],
      };
    case '/servicio-tecnico':
      return {
        badges: [
          { texto: `${m.listosEntregar} ${plural(m.listosEntregar, 'listo', 'listos')} para entregar`, tono: m.listosEntregar > 0 ? 'good' : 'info' },
          ...(m.atrasadosServicio > 0 ? [{ texto: `${m.atrasadosServicio} ${plural(m.atrasadosServicio, 'atrasado', 'atrasados')}`, tono: 'bad' as Tono }] : []),
        ],
      };
    case '/clientes':
      return { badges: [{ texto: `${m.clientes.toLocaleString('es-AR')} ${plural(m.clientes, 'cliente', 'clientes')}`, tono: 'info' }] };
    case '/compras':
      return { badges: m.comprasPendientes > 0 ? [{ texto: `${m.comprasPendientes} ${plural(m.comprasPendientes, 'pendiente', 'pendientes')}`, tono: 'warn' }] : [] };
    case '/canje':
      return { badges: m.enCanje > 0 ? [{ texto: `${m.enCanje} en canje`, tono: 'info' }] : [] };
    default:
      return { badges: [] };
  }
}
...
function TarjetaPrincipal({ mod, m }: { mod: Modulo; m: MetricasAccesos }) {
  const { badges, nota } = datosDe(mod.href, m);
  return (
    <Link href={mod.href} aria-label={mod.titulo} className={`${CARD_BASE} min-h-[132px] flex-col justify-between overflow-hidden p-4 sm:p-5`}>
      ...
      <div className="relative flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1 pt-0.5">
          <p className="font-display text-[15px] font-semibold leading-tight tracking-tight sm:text-base">{mod.titulo}</p>
          <p className="mt-0.5 text-xs leading-snug text-muted dark:text-dark-text-secondary">{mod.desc}</p>
        </div>
...
function TarjetaSecundaria({ mod, m }: { mod: Modulo; m: MetricasAccesos }) {
  const { badges } = datosDe(mod.href, m);
  return (
    <Link href={mod.href} aria-label={mod.titulo} className={`${CARD_BASE} items-center gap-3 p-3.5`}>
      <IlustracionModulo nombre={mod.ilustracion} className={`h-12 w-12 shrink-0 ${ILUS_ANIM}`} />
      <div className="min-w-0 flex-1">
        <p className="text-sm font-semibold leading-tight tracking-tight">{mod.titulo}</p>
        <p className="mt-0.5 truncate text-[11px] leading-snug text-muted dark:text-dark-text-secondary">{mod.desc}</p>
        {badges.length > 0 && (
...
function TarjetaAccion({ mod }: { mod: Modulo }) {
  return (
    <Link
      href={mod.href}
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        <p className="text-sm font-semibold leading-tight tracking-tight">{mod.titulo}</p>
        <p className="mt-0.5 text-[11px] leading-snug text-muted dark:text-dark-text-secondary">{mod.desc}</p>
      </div>
  ...
  return (
    <section aria-labelledby="accesos-titulo" className="flex flex-col gap-4 animate-fade-in-up" style={{ animationDelay: '240ms' }}>
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
        {OPERACION.map((mod) => (
          <TarjetaPrincipal key={mod.href} mod={mod} m={metricas} />
        ))}
      </div>

      {/* Compras y finanzas — accesos secundarios */}
      <Subtitulo>Compras y finanzas</Subtitulo>
      <div className="grid grid-cols-1 gap-2.5 sm:grid-cols-2">
        {FINANZAS.map((mod) => (
          <TarjetaSecundaria key={mod.href} mod={mod} m={metricas} />
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 gap-2.5 sm:grid-cols-2">
        {ACCIONES.map((mod) => (
          <TarjetaAccion key={mod.href} mod={mod} />
        ))}
      </div>
    </section>
  );
