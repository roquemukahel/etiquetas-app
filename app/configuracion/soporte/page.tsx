...
export default function Soporte() {
  ...
  const handleEnviar = async () => {
    ...
    try {
      ...
      setError(t('No pudimos enviar tu mensaje. Probá de nuevo en un rato.'));
    } finally {
      setEnviando(false);
    }
  };
  ...
  if (enviado) {
    return (
      <main className="flex min-h-screen flex-col items-center justify-center gap-4 px-6 text-center">
        ...
        <h1 className="text-xl font-display font-semibold">{t('¡Gracias por escribirnos!')}</h1>
        <p className="text-sm text-muted dark:text-dark-text-secondary max-w-xs">
          {t('Recibimos tu mensaje y te vamos a responder a la brevedad.')}
        </p>
  ...
  return (
    <main className="flex min-h-screen flex-col px-6 py-6 gap-4">
      <header className="flex items-center gap-3">
        ...
        <span className="text-lg font-medium">{t('Soporte')}</span>
      </header>

      <p className="text-sm text-muted dark:text-dark-text-secondary">
        {t('¿Tuviste algún problema, tenés alguna sugerencia o querés dejarnos un mensaje? Escribinos acá abajo.')}
      </p>
      ...
      <div className="flex flex-col gap-3">
        ...
        <div>
          ...
          {email.trim().length > 0 && !emailValido && (
            <p className="text-xs text-bad mt-1">{t('Ese email no parece válido.')}</p>
          )}
        </div>
        <Campo label={t('Teléfono (opcional)')} valor={telefono} onChange={setTelefono} />
        <div>
          <label className="text-xs text-muted dark:text-dark-text-secondary block mb-1">{t('Mensaje')}</label>
          <textarea
            value={mensaje}
            onChange={(e) => setMensaje(e.target.value)}
            rows={5}
            placeholder={t('Contanos qué pasó o qué te gustaría que agreguemos...')}
            className="w-full bg-white dark:bg-dark-surface border border-border dark:border-dark-border rounded-xl px-4 py-3 text-sm"
          />
        </div>
      </div>
      ...
      >
        {enviando ? t('Enviando...') : t('Enviar mensaje')}
      </button>
    </main>
  );
}
