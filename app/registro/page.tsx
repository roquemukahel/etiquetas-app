...
export default function Registro() {
  ...
  // Si el registro se corta justo después de crear la cuenta pero antes de
  // crear el negocio (falla de red, RPC caído, etc.), la persona queda
  // logueada pero sin negocio — sin esto, no tenía forma de volver a
  // intentarlo: /registro la mandaba de nuevo al formulario de alta de
  // cuenta (que falla porque el mail ya existe) y "/" le mostraba un panel
  // vacío para siempre.
  const [cuentaSinNegocio, setCuentaSinNegocio] = useState<'revisando' | 'no' | 'si'>('revisando');
  const [nombreNegocioRecuperar, setNombreNegocioRecuperar] = useState('');
  ...
  useEffect(() => {
    (async () => {
      ...
      setCuentaSinNegocio('si');
    })();
  }, []);

  const completarNegocio = async (e: React.FormEvent) => {
    e.preventDefault();
    setError(null);
    setCargando(true);
