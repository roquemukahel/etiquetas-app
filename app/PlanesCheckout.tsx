...
export default function PlanesCheckout({
  ...
  const t = useT();
  const labelMensual = textoBotonMensual ?? t('Suscribirme — Mensual');
  const labelAnual = textoBotonAnual ?? t('Suscribirme — Anual');
  ...
  if (!PAGOS_CON_TARJETA_DISPONIBLES) {
    return (
      <div className="w-full rounded-2xl border border-dashed border-border dark:border-dark-border p-4 flex flex-col gap-1 text-center">
        <p className="text-sm font-medium text-muted dark:text-dark-text-secondary">{t('Pago con tarjeta — no disponible por el momento')}</p>
        <p className="text-xs text-muted dark:text-dark-text-secondary">
          {t('Estamos terminando de habilitar esta opción (US$12.50/mes · US$125/año). Mientras tanto, podés suscribirte pagando con USDT (cripto) o por transferencia bancaria (Argentina) más abajo.')}
        </p>
      </div>
    );
  }
