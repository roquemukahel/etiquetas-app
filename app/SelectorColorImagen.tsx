      ...
      {colores.map((c, i) => {
        ...
        return (
          <button
            ...
            title={c.nombre}
            aria-pressed={sel}
            className={`group relative flex w-[92px] sm:w-[104px] flex-col items-center gap-1 rounded-2xl border p-1.5 pt-2 transition-colors ${
              sel
          ...
          >
            <span className="animate-flotar" style={{ animationDelay: `${(i % 3) * 0.4 + (i >= 3 ? 0.2 : 0)}s` }}>
              ...
              <img
                ...
                alt={`iPhone ${c.nombre}`}
                loading="lazy"
                className="h-20 sm:h-24 w-auto object-contain drop-shadow-md transition-transform duration-300 ease-out group-hover:scale-110"
              />
            </span>
